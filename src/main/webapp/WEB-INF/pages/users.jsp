...
<!-- Add Or Edit User  -->
<h2>
    <c:if test="${!empty user.name}"><spring:message text="Edit existing user"/></c:if>
    <c:if test="${empty user.name}"><spring:message text="Add new user"/></c:if>
</h2>
    ...
    <table>
        <c:if test='${!empty user.name}'>
            <tr>
                ...
                <td><form:input path="id" readonly="true" size="8" disabled="true"/><form:hidden
                        path="id"/></td>
            </tr>
        </c:if>
        <tr>
        ...
        <tr>
            <td colspan="2">
                <c:if test="${!empty user.name}"><input type="submit"
                                                        value="<spring:message text="Edit User"/>"/></c:if>
                <c:if test="${empty user.name}"><input type="submit"
                                                       value="<spring:message text="Add User"/>"/></c:if>
            </td>
        </tr>
    </table>
</form:form>
...
<ul class="pagination">
    <c:if test="${page > 1}">
        ...
        <li><a href="<c:url value="/users"><c:param name="page" value="${page - 1}"/>${page - 1}</c:url>">&#60;</a>
        </li>
    </c:if>
